    ...
    <div className="fixed inset-0 z-[80] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md bg-white rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
        <div className="p-5 border-b border-slate-100 flex items-center justify-between">
           <h3 className="text-lg font-bold text-slate-900">{category ? 'Edit Category' : 'Add Category'}</h3>
           <button onClick={onClose} className="p-1.5 hover:bg-slate-50 rounded-xl text-slate-400"><X size={20} /></button>
        </div>
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
           <div>
              ...
              <input
                required
                className="w-full bg-white border border-slate-200 rounded-xl py-2.5 px-4 text-xs font-medium focus:ring-1 focus:ring-primary focus:border-primary transition-all outline-none"
           ...
           <div className="flex gap-3">
              <button onClick={onClose} type="button" className="flex-1 py-2.5 bg-white text-slate-500 rounded-xl font-bold text-[10px] uppercase tracking-widest border border-slate-200">Cancel</button>
              <button
                type="submit"
                disabled={loading}
    ...
    <div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-2xl bg-white rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
        <div className="p-6 border-b border-slate-100 flex items-center justify-between">
           <h3 className="text-lg font-bold text-slate-900">{product ? 'Edit Product' : 'Add Product'}</h3>
           <button onClick={onClose} className="p-1.5 hover:bg-slate-50 rounded-xl text-slate-400"><X size={20} /></button>
        </div>

        <form onSubmit={handleSubmit} className="p-6 max-h-[70vh] overflow-y-auto space-y-5 no-scrollbar">
           {/* Image Upload */}
           <div className="flex flex-col items-center justify-center p-5 border-2 border-dashed border-slate-200 rounded-2xl bg-slate-50 hover:border-primary/50 transition-all cursor-pointer group"
                ...
                {imagePreview ? (
                    ...
                    <div className="text-center">
                        <div className="w-12 h-12 bg-white rounded-xl flex items-center justify-center mx-auto mb-2 shadow-sm text-slate-400 group-hover:text-primary transition-colors">
                            <ImageIcon size={24} />
                        </div>
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="col-span-2">
                 ...
                 <input
                  required className="w-full bg-white border border-slate-200 rounded-xl py-2.5 px-4 text-xs font-medium focus:ring-1 focus:ring-primary focus:border-primary transition-all outline-none"
              ...
              <div>
                 ...
                 <select
                  required
                  className="w-full bg-white border border-slate-200 rounded-xl py-2.5 px-4 text-xs font-medium focus:ring-1 focus:ring-primary focus:border-primary transition-all outline-none capitalize"
              ...
              <div>
                 <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1.5">Selling Price (KES)</label>
                 <input
                  required type="number" step="0.01"
                  className="w-full bg-white border border-slate-200 rounded-xl py-2.5 px-4 text-xs font-extrabold text-primary focus:ring-1 focus:ring-primary focus:border-primary transition-all outline-none"
                  placeholder="0.00"
              ...
              <div>
                 <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1.5">Original Price (KES)</label>
                 <input
                  type="number" step="0.01"
                  className="w-full bg-white border border-slate-200 rounded-xl py-2.5 px-4 text-xs font-medium text-slate-400 focus:ring-1 focus:ring-primary focus:border-primary transition-all outline-none"
                  placeholder="0.00"
              ...
              <div className="col-span-2">
                 <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1.5">Product Description</label>
                 <textarea
                  className="w-full bg-white border border-slate-200 rounded-xl py-2.5 px-4 text-xs font-medium focus:ring-1 focus:ring-primary focus:border-primary transition-all outline-none min-h-[80px]"
           ...
           <div className="flex flex-col gap-3 p-4 bg-slate-50 rounded-xl">
              <div className="flex items-center gap-3">
                 ...
                 <p className="text-xs font-bold text-slate-700">Product Visible</p>
              </div>
              <div className="flex items-center gap-3">
                 <StatusToggle active={formData.is_available} onToggle={() => setFormData({...formData, is_available: !formData.is_available})} />
                 <p className="text-xs font-bold text-slate-700">In Stock</p>
              </div>
           </div>
        </form>

        <div className="p-6 bg-slate-50 border-t border-slate-100 flex gap-3">
           <button onClick={onClose} type="button" className="flex-1 py-3 bg-white text-slate-500 rounded-xl font-bold text-[10px] uppercase tracking-widest border border-slate-200">Cancel</button>
           <button
    ...
    <div className="space-y-5 pb-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <button
            ...
            className="flex items-center gap-1.5 px-4 py-2 bg-white border border-slate-200 rounded-xl text-slate-600 font-bold text-[10px] uppercase tracking-widest hover:bg-slate-50 transition-all shadow-sm"
          >
      ...
      <div className="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden p-5">
        <div className="flex items-center justify-between mb-4">
           <h3 className="text-sm font-bold text-slate-900 px-1">Liquor Categories</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-3">
           {loading && categories.length === 0 ? (
              ...
              <div className="col-span-full py-6 text-center border-2 border-dashed border-slate-100 rounded-xl text-slate-400 text-xs font-medium">No categories yet.</div>
           ) : (
             categories.map(cat => (
               <div key={cat.id} className="group p-3 bg-white border border-slate-100 rounded-xl hover:border-primary/20 hover:shadow-md transition-all relative overflow-hidden">
                  <h4 className="text-[11px] font-extrabold text-slate-900 uppercase tracking-wider mb-2">{cat.name}</h4>
                  <div className="flex items-center gap-1">
                     <button onClick={() => { setEditingCategory(cat); setCategoryModalOpen(true); }} className="px-2 py-1 border border-slate-200 rounded-lg text-[8px] font-bold text-slate-500 hover:bg-slate-50 transition-all">Edit</button>
                     <button onClick={() => handleDeleteCategory(cat.id)} className="px-2 py-1 border border-slate-200 rounded-lg text-[8px] font-bold text-red-400 hover:bg-red-50 transition-all">Delete</button>
                  </div>
               </div>
             ))
           )}
      ...
      <div className="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden flex flex-col">
        <div className="p-5 border-b border-slate-50 flex items-center justify-between">
           ...
           <button className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-50 text-slate-500 rounded-xl font-bold text-[9px] uppercase tracking-widest hover:bg-slate-100 transition-all">
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-slate-50">
              {loading && products.length === 0 ? (
                ...
                <tr><td colSpan="6" className="px-5 py-12 text-center text-slate-400 text-xs font-medium">No products found.</td></tr>
              ) : (
                filteredProducts.map((product) => (
                  <tr key={product.id} className="hover:bg-slate-50/50 transition-all group">
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-3">
                        <div className="w-9 h-9 rounded-xl bg-slate-50 flex items-center justify-center text-slate-300 overflow-hidden border border-slate-100">
                        ...
                        <div>
                          ...
                          <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest mt-0.5">SKU: {product.sku || '---'}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-5 py-3.5"><span className="text-[9px] font-bold text-slate-500 uppercase tracking-widest">{product.category_name || product.category}</span></td>
                    <td className="px-5 py-3.5 text-xs font-extrabold text-primary">KES {parseFloat(product.price).toLocaleString()}</td>
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-1.5">
                        <span className={`text-xs font-extrabold ${product.stock <= product.low_stock_threshold ? 'text-red-500' : 'text-green-600'}`}>{product.stock}</span>
                    ...
                    <td className="px-5 py-3.5">
                       <div className="flex items-center gap-2">
                          ...
                          <span className={`text-[9px] font-bold uppercase tracking-widest ${product.is_active ? 'text-green-600' : 'text-slate-400'}`}>{product.is_active ? 'Live' : 'Hidden'}</span>
                       </div>
                    </td>
                    <td className="px-5 py-3.5 text-right">
                       <button onClick={() => { setEditingProduct(product); setProductModalOpen(true); }} className="w-8 h-8 flex items-center justify-center text-slate-400 hover:text-primary hover:bg-primary-light rounded-xl transition-all"><Edit2 size={14} /></button>
                    </td>
                  </tr>
                ))
              )}
