...
const InventoryStat = ({ title, value, icon: Icon, color, loading }) => (
  <div className="bg-white dark:bg-slate-900 p-3.5 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-sm hover:shadow-md transition-all">
    ...
    {loading ? (
      <div className="h-5 w-12 bg-slate-100 dark:bg-slate-800 animate-pulse rounded-lg" />
    ) : (
    ...
    <div className="space-y-5 pb-8">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <InventoryStat
          ...
          icon={TrendingDown}
          color={{bg: 'bg-red-50 dark:bg-red-900/20', text: 'text-red-500 dark:text-red-400'}}
          loading={loading}
        />
        <InventoryStat
          title="Low Stock"
          ...
          icon={AlertTriangle}
          color={{bg: 'bg-orange-50 dark:bg-orange-900/20', text: 'text-orange-500 dark:text-orange-400'}}
          loading={loading}
        />
        <InventoryStat
          title="Active Items"
      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-sm overflow-hidden flex flex-col">
        <div className="p-4 border-b border-slate-50 dark:border-slate-800 flex flex-col md:flex-row md:items-center justify-between gap-3">
           <div className="relative flex-1 max-w-md">
              ...
              <input
                ...
                className="w-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl py-2.5 pl-11 pr-4 text-xs font-medium text-slate-900 dark:text-white focus:ring-1 focus:ring-primary focus:border-primary transition-all outline-none"
                value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
              />
           </div>
           <button onClick={fetchData} className="w-8 h-8 bg-slate-50 dark:bg-slate-800 text-slate-400 dark:text-slate-500 rounded-xl flex items-center justify-center hover:bg-slate-100 dark:hover:bg-slate-700 transition-all">
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-slate-50 dark:divide-slate-800">
              {loading && items.length === 0 ? (
                ...
                <tr><td colSpan="6" className="px-5 py-12 text-center text-slate-400 dark:text-slate-500 text-xs font-medium">No items found.</td></tr>
              ) : (
                filteredItems.map(item => (
                  <tr key={item.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-all group">
                    <td className="px-5 py-3.5">
                      ...
                      <p className="text-[9px] text-slate-400 dark:text-slate-500 font-bold uppercase tracking-widest mt-0.5">{item.category_name || 'Uncategorized'}</p>
                    </td>
                    <td className="px-5 py-3.5">
                      <span className="font-mono text-[10px] text-slate-400 dark:text-slate-500 uppercase tracking-tighter">{item.sku || '---'}</span>
                    </td>
                    ...
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-2">
                         <input
                           type="number"
                           className={`w-16 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg px-2 py-1 text-xs font-extrabold focus:ring-1 focus:ring-primary outline-none transition-all ${item.stock <= item.low_stock_threshold ? 'text-red-500' : 'text-slate-900 dark:text-white'}`}
                           value={item.stock}
                    ...
                    <td className="px-5 py-3.5">
                      <span className={`px-2 py-0.5 rounded text-[9px] font-bold uppercase tracking-wider ${
                        !item.is_available || item.stock === 0 ? 'bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400' :
                        item.stock <= item.low_stock_threshold ? 'bg-orange-50 dark:bg-orange-900/20 text-orange-600 dark:text-orange-400' :
                    ...
                    <td className="px-5 py-3.5 text-right">
                       <Edit2 size={12} className="text-slate-300 dark:text-slate-600 group-hover:text-primary cursor-pointer ml-auto" />
                    </td>
                  </tr>
                ))
              )}
      ...
      <div className="bg-primary/5 dark:bg-primary/10 rounded-3xl p-4 border border-primary/10 flex items-center gap-3">
