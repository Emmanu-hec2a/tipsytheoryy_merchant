...
const StatCard = ({ title, value, icon: Icon, color, loading }) => (
  <div className="bg-white p-3.5 rounded-3xl shadow-sm border border-slate-100 hover:shadow-md transition-all">
    <div className="flex items-center gap-2.5 mb-1.5">
      <div className={`w-7 h-7 rounded-xl ${color.bg} ${color.text} flex items-center justify-center shrink-0`}>
        <Icon size={14} />
      </div>
      <p className="text-slate-500 text-[8px] font-bold uppercase tracking-widest truncate">{title}</p>
    </div>
    {loading ? (
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-slate-500 text-xs">Welcome back! Here's what's happening today.</p>
        </div>
        <div className="flex items-center gap-2">
           <div className="flex items-center gap-2 bg-white px-3.5 py-2 rounded-xl border border-slate-200 text-slate-600 font-medium text-xs shadow-sm">
              <Calendar size={16} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-slate-50">
                {loading ? (
                   [...Array(5)].map((_, i) => (
                    <tr key={i}>
                      <td colSpan="5" className="px-5 py-3.5"><div className="h-3.5 w-full bg-slate-50 animate-pulse rounded" /></td>
                    </tr>
                   ))
                ) : recentOrders.length === 0 ? (
                  <tr>
                    <td colSpan="5" className="px-5 py-8 text-center text-slate-400 text-xs font-medium">No recent orders yet</td>
                  </tr>
                ) : (
                    ...
                    <tr key={order.id} className="hover:bg-slate-50 transition-all group">
                      ...
                      <td className="px-5 py-3.5">
                        <div className="flex items-center gap-2.5">
                          ...
                          <span className="text-xs font-medium text-slate-600 truncate max-w-[120px]">{order.customer_email || order.customer_name}</span>
                        </div>
                      </td>
        ...
        <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-4">
           <h3 className="text-sm font-bold text-slate-900 mb-4">Inventory Status</h3>
           <div className="space-y-3">
              {[
                ...
                <Link to={item.link} key={i} className="flex items-center justify-between p-3 rounded-2xl border border-slate-50 hover:border-slate-100 transition-all cursor-pointer group">
                  <div className="flex items-center gap-2.5">
                    <div className={`w-8 h-8 rounded-xl ${item.bg} ${item.color} flex items-center justify-center`}>
                       <item.icon size={16} />
                    </div>
                    <div>
                        <p className="text-[11px] font-bold text-slate-900 leading-none mb-1">{item.label}</p>
                        <p className="text-[8px] text-slate-400">Inventory Sync</p>
                    </div>
                  </div>
           ...
           <div className="mt-6 bg-primary-light p-3.5 rounded-2xl">
